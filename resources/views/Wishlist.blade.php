...
@foreach($real as $item)
    <div class="col">
        <div class="card position-relative">
            <!-- Wishlist Button -->
            @auth
                <form action="{{ in_array($item->id, $wishlist) ? route('wishlist.remove', $item->id) : route('wishlist.add', $item->id) }}"
                      method="POST" style="position: absolute; top: 10px; right: 10px; z-index: 10;">
                    @csrf
                    @if(in_array($item->id, $wishlist))
                        <button type="submit" class="btn btn-sm btn-light">
                            <i class="fas fa-heart text-danger"></i> {{-- Filled red heart --}}
                        </button>
                    @else
                        <button type="submit" class="btn btn-sm btn-light">
                            <i class="far fa-heart text-danger"></i> {{-- Transparent heart --}}
                        </button>
                    @endif
                </form>
            @endauth
            ...
            <a href="{{ route('real.show', ['real' => $item->id]) }}" class="card-link">
                ...
                <div class="card-body">
                    <p class="card-text">Real Type:
                        @if($item->real_type == 1) Residential
                        @elseif($item->real_type == 2) Commercial Land
                        @elseif($item->real_type == 3) Other
                        @endif
                    </p>
